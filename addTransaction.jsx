...
const AddTrans = ({ID,parameters}) => {
  ...
  const currencies = [
    ...
    // Add similar entries for other cryptocurrencies
  ];
  ...
  useEffect(() => {
    ...
    // Fetch the current price when selectedCoin changes
    if (selectedCoin) {
      fetchCryptoInfo(selectedCoin).then((cryptoInfo) => {
        ...
        if (cryptoInfo && cryptoInfo.price_usd) {
          ...
          // Set the pricePerCoin dynamically
    ...
    <div>
      ...
      <Dialog open={isTransactionDetailsOpen} onClose={handleClose} fullWidth maxWidth="sm">
        ...
        <DialogContent>
          ...
          {/* Display the fetched price directly */}
      {/* {selectedCoin && (
...
export default AddTrans;
  ...
  // useEffect(() => {
  //   const fetchPricesForAllCoins = async () => {
  //     try {
  //       const prices = await Promise.all(
  //         currencies.map(async (currency) => {
  //           const response = await fetch(`https://api.coinlore.net/api/tickers/?symbol=${currency.symbol}`);
  //           const data = await response.json();
  //           const price = data?.data?.[0]?.price_usd;
  
  //           if (price) {
  //             return { symbol: currency.symbol, price };
  //           } else {
  //             console.error(`Price not available for ${currency.name} (${currency.symbol}).`);
  //             return { symbol: currency.symbol, price: null };
  //           }
  //         })
  //       );
  
  //       // Update state variables based on fetched prices
  //       prices.forEach(({ symbol, price }) => {
  //         // Assuming you have a state variable for each coin's price, update accordingly
  //         // For example: setBitcoinPrice(price) or setEthereumPrice(price)
  ...
  //   // Call the fetchPricesForAllCoins function
  //   fetchPricesForAllCoins();
  
  //   // Recalculate total value whenever quantity or pricePerCoin changes
  //   handleTotalValueChange();
  // }, [selectedCoin, quantity, pricePerCoin]);